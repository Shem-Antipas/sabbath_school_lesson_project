...
// .wrangler/tmp/bundle-nxaPlA/checked-fetch.js
...
app.get("/lesson/:path{.+}", async (c) => {
  const UPSTREAM2 = "https://sabbath-school.adventech.io/api/v1";
  let path = c.req.param("path");
  const cleanPath = path.replace(/-/g, "/").replace(/^\//, "");
  const parts = cleanPath.split("/");
  if (parts.length < 3) {
    return c.json({ error: "Invalid Path", message: `Need lang/quarterly/lesson, got: ${cleanPath}` }, 400);
  }
  const [lang, qId, lId] = parts;
  const finalUrl = `${UPSTREAM2}/${lang}/quarterlies/${qId}/lessons/${lId}/read/index.json`;
  console.log("\u{1F310} FETCHING FROM ADVENTECH:", finalUrl);
  try {
    const response = await fetch(finalUrl);
    if (!response.ok) {
      return c.json({
        error: "Not Found",
        message: `Adventech API returned ${response.status}. This lesson might not be uploaded yet.`,
    ...
    if (!contentType || !contentType.includes("application/json")) {
      return c.json({
        ...
        message: "Server returned HTML (404 page) instead of JSON data.",
        url: finalUrl
      }, 500);
    }
    const data = await response.json();
    ...
    return c.json({ error: "Proxy Error", details: e.message }, 500);
  }
});
var src_default = app;
