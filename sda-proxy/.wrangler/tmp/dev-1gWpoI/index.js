...
// .wrangler/tmp/bundle-L3K8Iv/checked-fetch.js
...
app.get("/quarterly/:lang/:quarterly/:lesson/:day", async (c) => {
  ...
  console.log(`\u{1F4D6} Fetching Day Read: ${url}`);
  try {
...
app.get("/quarterly/:lang/:quarterly/:lesson", async (c) => {
  ...
  console.log(`\u{1F4D1} Fetching Lesson Index: ${url}`);
  try {
    const response = await fetch(url);
    if (!response.ok) {
...
app.get("/quarterly/:lang/:quarterlyId", async (c) => {
  ...
  console.log(`\u{1F4DA} Fetching Quarterly List: ${url}`);
  try {
    const response = await fetch(url);
