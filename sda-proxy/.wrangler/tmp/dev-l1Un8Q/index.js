...
// .wrangler/tmp/bundle-qXQQVg/checked-fetch.js
...
// src/index.js
var app = new Hono2();
var UPSTREAM = "https://sabbath-school.adventech.io/api/v1";
app.use("/*", cors({
  origin: "*",
...
app.get("/quarterlies/:lang", async (c) => {
  try {
    ...
    console.log(`\u{1F3E0} Fetching Main List: ${url}`);
...
app.get("/quarterly/:path{.+}", async (c) => {
  try {
    let path = c.req.param("path");
    if (path.startsWith("/")) path = path.substring(1);
    const parts = path.split("/");
    let lang, quarterlyId;
    if (parts.length >= 2) {
      lang = parts[0];
      quarterlyId = parts[1];
    } else {
    ...
    console.log(`\u{1F4DA} Fetching Lesson List: ${url}`);
    const response = await fetch(url);
    if (!response.ok) return c.json({ error: "Quarterly not found", url }, 404);
    const data = await response.json();
    return c.json(data);
  } catch (err) {
...
app.get("/lesson/:path{.+}", async (c) => {
  try {
    ...
    if (parts.length === 2) {
      ...
      return c.json({ error: "Path too short" }, 400);
    }
    const safeLId = lId.toString().padStart(2, "0");
    const finalUrl = `${UPSTREAM}/${lang}/quarterlies/${qId}/lessons/${safeLId}/read/index.json`;
    console.log("\u{1F680} Proxying to:", finalUrl);
    const response = await fetch(finalUrl);
    if (!response.ok) {
      return c.json({ error: "Adventech Error", status: response.status }, response.status);
    }
    const contentType = response.headers.get("content-type");
    if (!contentType || !contentType.includes("application/json")) {
      return c.json({ error: "Invalid Data", message: "Source returned HTML instead of JSON" }, 500);
    }
    const data = await response.json();
    return c.json(data);
  } catch (e) {
    return c.json({ error: "Proxy Crash", details: e.message }, 500);
  }
});
app.get("/lesson/:lang/:quarterly/:lesson", async (c) => {
  const { lang, quarterly, lesson } = c.req.param();
  const url = `https://sabbath-school.adventech.io/api/v1/${lang}/quarterlies/${quarterly}/lessons/${lesson}/index.json`;
  try {
    const response = await fetch(url);
    if (!response.ok) {
      return c.json({
        error: "Content not available on Adventech server yet",
    ...
    return c.json({ error: "Proxy failed to fetch data", details: e.message }, 500);
  }
});
